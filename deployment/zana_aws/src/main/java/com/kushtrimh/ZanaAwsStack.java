...
public class ZanaAwsStack extends Stack {
    ...
    public ZanaAwsStack(final Construct scope, final String id, final StackProps props) {
        super(scope, id, props);

        String zanaEnv = Optional.ofNullable(props).flatMap(p -> Optional.ofNullable(p.getTags()).map(tags -> tags.get("zanaEnv")))
                .orElse("prod");
        ...
        var hostedZoneId = StringParameter.valueForStringParameter(this, String.format("/zana/%s/hosted-zone-id", zanaEnv));
        var hostedZoneName = StringParameter.valueForStringParameter(this, String.format("/zana/%s/hosted-zone-name", zanaEnv));
    ...
    private Distribution createCloudFrontDistribution(String zanaEnv, RestApi restApi) {
        var certificateArn = StringParameter.valueForStringParameter(this, String.format("/zana/%s/certificate-arn", zanaEnv));
        var apiDomain = StringParameter.valueForStringParameter(this, String.format("/zana/%s/api-domain", zanaEnv));
        ...
        return Distribution.Builder.create(this, "zana-distribution")
                ...
                .defaultBehavior(BehaviorOptions.builder()
                        .cachePolicy(CachePolicy.Builder.create(this, "zana-distribution-cache-policy")
                                ...
                                .build())
                        .origin(HttpOrigin.Builder.create(apiDomainName).originPath("/" + zanaEnv).build())
                        .build())
                .enableLogging(true)
    ...
    private Function createZanaBooksDataLambda(String zanaEnv) {
        var lambdaSSMExtensionArn = StringParameter.valueForStringParameter(this, String.format("/zana/%s/lambda-ssm-extension-arn", zanaEnv));
        ...
        var lambdaRole = Role.Builder.create(this, "zana-books-lambda-role")
                ...
                .description("Allows lambda functions to retrieve parameters from AWS SSM. Intended to be by Zana book handler lambdas.")
    ...
    private RestApi createZanaRestApi(String zanaEnv, Function booksDataLambda) {
        ...
        var restApi = RestApi.Builder.create(this, "zana-books-api")
                ...
                .defaultCorsPreflightOptions(CorsOptions.builder()
                        ...
                        .allowOrigins(Cors.ALL_ORIGINS) // TODO: come from environment
                        .build())
        ...
        var apiGatewayCloudWatchRole = Role.Builder.create(this, "zana-api-gateway-cloudwatch-role")
                ...
                .managedPolicies(List.of(ManagedPolicy.fromAwsManagedPolicyName("service-role/AmazonAPIGatewayPushToCloudWatchLogs")))
                .build();
